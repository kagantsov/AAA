def open_menu():
    ...
    print('3. Сохранить отчет сводный отчет по департаментам в csv-файл.')
...
def user_input_and_check():
    ...
    if user_input.isdigit() and int(user_input) in {1, 2, 3}:
        ...
        print('\nВы выбрали несуществующую опцию. Попробуйте снова.')
...
def department_consolidated_report() -> list[dict]:
    ...
    for department, salary in department_data.items():
        row = [department, str(salary[0]), str(min(salary[1])),
               str(max(salary[1])), str(round(sum(salary[1]) / len(salary[1])))]
        result.append(row)
    return result
...
def demonstrate_department_consolidated_report():
    ...
    columns = ['Департамент', 'Численность сотрудников', 'Минимальная зарплата',
               'Максимальная зарплата', 'Средняя зарплата']
    print('\n{:<12}  {:>12}  {:>12}  {:>12}  {:>12}'.format(*columns))
    for department in data_consolidated_report:
        print('\n{:<12}  {:>12}  {:>24}  {:>22}  {:>16}'.format(*department))


def department_consolidated_report_to_csv():
    """Сохранение сводного отчета по департаменту в формате CSV"""
    data_consolidated_report = department_consolidated_report()
    columns = ['Департамент', 'Численность сотрудников', 'Минимальная зарплата', 'Максимальная зарплата',
               'Средняя зарплата']
...
def main():
    """Общая функция для задания, объединяющая функцию меню, чтения файла и каждую из трех доступных команд меню"""
    open_menu()
